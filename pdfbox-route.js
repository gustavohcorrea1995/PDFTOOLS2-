  ...
  app.post('/api/edit/native', async (req, res) => {
    ...
    try {
      ...
      for (const e of edits) {
        ...
        const original = String(e.originalText ?? '');
        const replacement = String(e.text ?? '');
        const originalField = original || '__NEW__';
        lines.push([
          page, b64(originalField), b64(replacement), x, y, Math.max(0.1,w), Math.max(0.1,h),
          e.deleted === true, Math.max(4, Number(e.fontSize) || 10), String(e.color || '#111111')
        ].join('\t'));
      }
      ...
      const jar = process.env.PDFBOX_JAR || '/opt/pdfbox/pdfbox-app-3.0.8.jar';
      const engine = process.env.PDFBOX_ENGINE_JAR || '/opt/pdfbox/pdfbox-engine.jar';
      if (!fs.existsSync(jar) || !fs.existsSync(engine)) throw new Error('Motor PDFBox não está instalado no servidor.');

      await new Promise((resolve, reject) => {
        execFile(java, ['-Xms64m','-Xmx768m','-cp',`${engine}:${jar}`,'NativePdfEditor',input,output,manifest],
          { timeout: 240000, maxBuffer: 8 * 1024 * 1024 }, (err, stdout, stderr) => {
            if (err) return reject(new Error(`${err.message}${stderr ? `: ${String(stderr).slice(0,2500)}` : ''}`));
            console.log(`PDFBox native editor: ${String(stdout).trim()}`);
            resolve();
          });
      });
      ...
      if (!stat.size) throw new Error('O PDFBox retornou um arquivo vazio.');
      res.status(200).set({
        'Content-Type':'application/pdf',
        'Content-Disposition':'attachment; filename="PDFTools2-editado.pdf"',
        'Content-Length':String(stat.size),
        'Cache-Control':'no-store, max-age=0',
        'X-PDFTools-Engine':'Apache-PDFBox-3.0.8-native'
      });
      fs.createReadStream(output).pipe(res);
      res.on('finish',()=>fs.rm(output,{force:true},()=>{}));
    } catch (e) {
      if (output) fs.rm(output,{force:true},()=>{});
      if (!res.headersSent) res.status(500).json({ error: e.message || 'Falha no motor PDFBox.' });
    } finally {
      if (manifest) fs.rm(manifest,{force:true},()=>{});
    }
  });
};
