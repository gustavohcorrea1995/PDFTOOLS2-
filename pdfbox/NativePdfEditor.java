...
public class NativePdfEditor {
    ...
    static double distance(Run r,Edit e,PDPage page)throws IOException{double pageH=page.getCropBox().getHeight();double targetY=pageH-e.y-e.h;double dx=0;if(r.x<e.x)dx=e.x-r.x;else if(r.x>e.x+e.w)dx=r.x-(e.x+e.w);double runWidth=0;try{runWidth=r.font==null?0:r.font.getStringWidth(r.text)/1000.0*r.fontSize;}catch(Exception ignored){}if(r.x>=e.x&&r.x<=e.x+e.w)dx=0;else if(r.x+runWidth>=e.x&&r.x+runWidth<=e.x+e.w)dx=0;double dy=Math.abs(r.y-targetY);return dx+dy*1.35;}
    static boolean near(Run r,Edit e,PDPage page)throws IOException{if(r==null)return false;double pageH=page.getCropBox().getHeight();double targetY=pageH-e.y-e.h;double dy=Math.abs(r.y-targetY);double dx=Math.abs(r.x-e.x);return dy<=24.0&&dx<=45.0;}
    static byte[] encode(PDFont font,String text)throws IOException{return font.encode(text);}
    static Edit findEdit(List<Edit> edits,String text,Run run,PDPage page,Set<Edit> used)throws IOException{if(run==null)return null;Edit best=null;double bestScore=Double.MAX_VALUE;for(Edit e:edits){if(used.contains(e))continue;if(e.original==null||e.original.isEmpty()||e.original.equals("__NEW__"))continue;if(text.indexOf(e.original)<0)continue;if(!near(run,e,page))continue;double score=distance(run,e,page);if(score<bestScore){best=e;bestScore=score;}}return best;}
    static void addBoldTokens(List<Object> out){out.add(COSInteger.get(2));out.add(Operator.getOperator("Tr"));out.add(new COSFloat(0.45f));out.add(Operator.getOperator("w"));}
    static void resetTextStyleTokens(List<Object> out){out.add(COSInteger.get(0));out.add(Operator.getOperator("Tr"));}
    static boolean replaceOnPage(PDDocument doc,PDPage page,List<Edit> edits)throws IOException{if(edits.isEmpty()||!page.hasContents())return false;Collector collector=new Collector();collector.processPage(page);List<Run> runs=collector.runs;int runIndex=0;boolean changed=false;Set<Edit> used=new HashSet<>();PDFStreamParser parser=new PDFStreamParser(page);List<Object> tokens=parser.parse();List<Object> out=new ArrayList<>(tokens.size());PDFont currentFont=null;for(Object token:tokens){if(token instanceof Operator){Operator op=(Operator)token;String name=op.getName();if("Tf".equals(name)&&out.size()>=2&&out.get(out.size()-2)instanceof COSName)currentFont=page.getResources().getFont((COSName)out.get(out.size()-2));if("Tj".equals(name)&&!out.isEmpty()&&out.get(out.size()-1)instanceof COSString&&currentFont!=null){COSString s=(COSString)out.get(out.size()-1);String text=decode(currentFont,s.getBytes());Run run=runIndex<runs.size()?runs.get(runIndex++):null;Edit hit=findEdit(edits,text,run,page,used);if(hit!=null){String newText=text.replace(hit.original,hit.deleted?"":hit.replacement);s.setValue(encode(currentFont,newText));if(hit.bold&&!hit.deleted)addBoldTokens(out);out.add(token);if(hit.bold&&!hit.deleted)resetTextStyleTokens(out);used.add(hit);changed=true;continue;}}if("TJ".equals(name)&&!out.isEmpty()&&out.get(out.size()-1)instanceof COSArray&&currentFont!=null){COSArray arr=(COSArray)out.get(out.size()-1);StringBuilder all=new StringBuilder();List<COSString> strings=new ArrayList<>();for(COSBase b:arr)if(b instanceof COSString){COSString cs=(COSString)b;strings.add(cs);all.append(decode(currentFont,cs.getBytes()));}String text=all.toString();Run run=runIndex<runs.size()?runs.get(runIndex++):null;Edit hit=findEdit(edits,text,run,page,used);if(hit!=null&&!strings.isEmpty()){String newText=text.replace(hit.original,hit.deleted?"":hit.replacement);strings.get(0).setValue(encode(currentFont,newText));for(int k=1;k<strings.size();k++)strings.get(k).setValue(new byte[0]);if(hit.bold&&!hit.deleted)addBoldTokens(out);out.add(token);if(hit.bold&&!hit.deleted)resetTextStyleTokens(out);used.add(hit);runIndex=Math.min(runs.size(),runIndex+Math.max(0,strings.size()-1));changed=true;continue;}runIndex=Math.min(runs.size(),runIndex+Math.max(0,strings.size()-1));}}out.add(token);}if(changed){PDStream newContents=new PDStream(doc);try(OutputStream os=newContents.createOutputStream(COSName.FLATE_DECODE)){new ContentStreamWriter(os).writeTokens(out);}page.setContents(newContents);}if(used.size()!=edits.size())System.out.println("NATIVE_MATCHED="+used.size()+" REQUESTED="+edits.size());return changed;}
    static void addNewText(PDDocument doc,PDPage page,Edit e)throws IOException{if(e.deleted||e.replacement==null||e.replacement.isEmpty())return;PDFont font=new org.apache.pdfbox.pdmodel.font.PDType1Font(Standard14Fonts.FontName.HELVETICA);float pageH=page.getCropBox().getHeight(),y=(float)(pageH-e.y-Math.max(8,e.h)),size=(float)(e.size>0?e.size:10);try(org.apache.pdfbox.pdmodel.PDPageContentStream cs=new org.apache.pdfbox.pdmodel.PDPageContentStream(doc,page,org.apache.pdfbox.pdmodel.PDPageContentStream.AppendMode.APPEND,true,true)){if(e.bold){cs.setRenderingMode(RenderingMode.FILL_STROKE);cs.setLineWidth(.45f);}cs.beginText();cs.setFont(font,size);cs.newLineAtOffset((float)e.x,y);cs.showText(e.replacement);cs.endText();}}
