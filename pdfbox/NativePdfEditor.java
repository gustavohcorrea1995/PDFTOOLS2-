...
public class NativePdfEditor {
    ...
    static class Run {
        String text; float x,y; PDFont font; float fontSize;
        Run(String text,float x,float y,PDFont font,float fontSize){this.text=text;this.x=x;this.y=y;this.font=font;this.fontSize=fontSize;}
    }
    ...
    static String decode(PDFont font,byte[] bytes)throws IOException{
        ...
        try(ByteArrayInputStream in=new ByteArrayInputStream(bytes)){
            while(in.available()>0){int code=font.readCode(in);String u=font.toUnicode(code);if(u!=null)out.append(u);}
        }
        return out.toString();
    }
    static boolean near(Run r,Edit e,PDPage page)throws IOException{
        ...
        boolean xInside=e.x>=r.x-20 && e.x<=r.x+runWidth+20;
        boolean yNear=Math.abs(r.y-targetY)<=Math.max(18,e.h*2.0);
        return xInside&&yNear;
    }
    static byte[] encode(PDFont font,String text)throws IOException{return font.encode(text);}

    static Edit findEdit(List<Edit> edits,String text,Run run,PDPage page)throws IOException{
        Edit best=null;double bestScore=Double.MAX_VALUE;
        for(Edit e:edits){
            if(e.original==null||e.original.isEmpty()||e.original.equals("__NEW__"))continue;
            int at=text.indexOf(e.original);
            if(at<0)continue;
        ...
        for(Object token:tokens){
            if(token instanceof Operator){
                ...
                if("Tj".equals(name)&&!out.isEmpty()&&out.get(out.size()-1)instanceof COSString&&currentFont!=null){
                    ...
                    if(hit!=null){String newText=text.replace(hit.original,hit.deleted?"":hit.replacement);s.setValue(encode(currentFont,newText));changed=true;}
                }else if("TJ".equals(name)&&!out.isEmpty()&&out.get(out.size()-1)instanceof COSArray&&currentFont!=null){
                    COSArray arr=(COSArray)out.get(out.size()-1);StringBuilder all=new StringBuilder();List<COSString> strings=new ArrayList<>();
                    for(COSBase b:arr)if(b instanceof COSString){COSString cs=(COSString)b;strings.add(cs);all.append(decode(currentFont,cs.getBytes()));}
    ...
    public static void main(String[] args)throws Exception{
        ...
        Path input=Paths.get(args[0]),output=Paths.get(args[1]),manifest=Paths.get(args[2]);List<Edit> edits=readManifest(manifest);
        try(PDDocument doc=Loader.loadPDF(input.toFile())){
            Map<Integer,List<Edit>> byPage=new LinkedHashMap<>();for(Edit e:edits)byPage.computeIfAbsent(e.page,k->new ArrayList<>()).add(e);int nativeChanged=0,newCount=0;
            for(Map.Entry<Integer,List<Edit>> entry:byPage.entrySet()){
                int idx=entry.getKey()-1;if(idx<0||idx>=doc.getNumberOfPages())continue;PDPage page=doc.getPage(idx);List<Edit> newOnes=new ArrayList<>(),existing=new ArrayList<>();
                ...
                if(replaceOnPage(doc,page,existing))nativeChanged++;for(Edit e:newOnes){addNewText(doc,page,e);newCount++;}
            }
            doc.save(output.toFile());System.out.println("NATIVE_CHANGED="+nativeChanged+" NEW="+newCount);
        }
    }
}
