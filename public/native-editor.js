(() => {
  ...
  document.querySelectorAll('[data-mode]').forEach(btn => btn.addEventListener('click', () => { mode=btn.dataset.mode; document.querySelectorAll('[data-mode]').forEach(b=>b.classList.toggle('active',b===btn)); setStatus(mode==='text'?'Clique em uma área da página para adicionar texto.':`Modo: ${btn.textContent.trim()}`); }));

  async function loadPdf(selectedFile){
    if(!selectedFile)return;
    if(!/\.pdf$/i.test(selectedFile.name)){setStatus('Selecione um arquivo PDF.');return;}
    file=selectedFile;
    ...
    try{
      ...
      currentPage=1; renderPage(1,data.thumbnails?.[0]); saveBtn.disabled=false;
  ...
  function renderPage(page,thumb){
    ...
    const wrap=document.createElement('div');wrap.className='native-page-wrap';wrap.style.cssText='position:relative;display:inline-block;line-height:0;flex:0 0 auto;';
    const img=document.createElement('img');img.className='native-page';img.alt=`Página ${page}`;img.src=thumb||`/api/preview/${encodeURIComponent(fileId)}/${page}`;wrap.appendChild(img);stage.appendChild(wrap);
  ...
  function applyBoxVisual(item,box,sx,sy){
    ...
    /* Texto original nunca é duplicado pela camada HTML. */
    ...
    box.style.fontWeight=item.bold?'700':'400';box.style.fontStyle=item.italic?'italic':'normal';box.style.textDecoration=item.underline?'underline':'none';
    box.dataset.changed=changed?'true':'false';box.style.boxShadow=changed?'0 0 0 1px rgba(255,255,255,.9) inset':'none';
  }
  ...
  function renderTextObjects(page,img,wrap){
    ...
    textBoxes.filter(t=>Number(t.page)===page&&t.deleted!==true).forEach(item=>{
      const box=document.createElement('div');box.className='native-text-object';box.dataset.id=item.id;box.title='Clique para editar este texto';
  ...
  saveBtn.addEventListener('click',async()=>{
    if(!fileId)return;try{saveBtn.disabled=true;saveBtn.textContent='Salvando…';setStatus('Aplicando as alterações no PDF original…');
      const annotations=textBoxes.filter(item=>String(item.id||'').startsWith('pnew-')||item.changed===true||String(item.text??'')!==String(item.originalText??'')||item.deleted===true).map(item=>({id:item.id,page:item.page,pdfX:Number(item.pdfX),pdfY:Number(item.pdfY),pdfWidth:Number(item.pdfWidth),pdfHeight:Number(item.pdfHeight),text:String(item.text??''),fontSize:Number(item.fontSize)||Number(item.pdfHeight)||12,color:item.color||'#111111',bold:item.bold===true,italic:item.italic===true,underline:item.underline===true,deleted:item.deleted===true}));
      if(!annotations.length){setStatus('Nenhuma alteração para salvar.');return;}
      const fd=new FormData();fd.append('fileId',fileId);fd.append('annotations',JSON.stringify(annotations));const response=await fetch('/api/edit/annotate',{method:'POST',body:fd});if(!response.ok){let message='Falha ao salvar o PDF.';try{const data=await response.json();if(data?.error)message=data.error;}catch(_){}throw new Error(message);}const blob=await response.blob();if(!blob.size)throw new Error('O servidor retornou um PDF vazio.');const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='PDFTools2-editado.pdf';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1500);dirty=false;setStatus('PDF salvo com sucesso. A alteração substituiu somente a área selecionada.');
    }catch(e){console.error(e);setStatus(`Erro ao salvar: ${e.message||e}`);}finally{saveBtn.disabled=false;saveBtn.textContent='Salvar PDF';}
  });
})();
